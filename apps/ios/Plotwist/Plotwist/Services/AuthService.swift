...
class AuthService {
    ...
    func signUp(email: String, password: String, username: String) async throws {
        guard let url = URL(string: "\(API.baseURL)/users") else {
            throw AuthError.invalidURL
        }
    ...
    func checkEmailAvailable(email: String) async throws -> Bool {
        guard let url = URL(string: "\(API.baseURL)/users/available/email?email=\(email)") else {
            throw AuthError.invalidURL
        }
    ...
    func checkUsernameAvailable(username: String) async throws -> Bool {
        guard let url = URL(string: "\(API.baseURL)/users/available/username?username=\(username)") else {
            throw AuthError.invalidURL
        }
