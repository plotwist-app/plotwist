...
const {query: queryOptions} = options ?? {};
  ...
  const queryKey =  queryOptions?.queryKey ?? getGetImportImportIdQueryKey(importId);
   ...
   return  { queryKey, queryFn, enabled: !!(importId), ...queryOptions} as UseSuspenseQueryOptions<Awaited<ReturnType<typeof getImportImportId>>, TError, TData> & { queryKey: QueryKey }
}
