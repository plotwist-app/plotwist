...
export function OnboardingWelcome({ lang }: { lang: string }) {
  ...
  const title = dictionary?.welcome_title || 'Track, discover and never miss a title'
  const subtitle = dictionary?.welcome_subtitle || 'Join the community of movie and TV show lovers.'
  const cta = dictionary?.get_started || 'Get started'


  const { data } = useQuery({
    queryKey: ['onboarding-welcome-posters', language],
    queryFn: () => tmdb.movies.discover({
  ...
  const posters = data?.results.slice(0, 20).map(m => m.poster_path).filter(Boolean) as string[] ?? []
  ...
  const Column = ({ images, duration, delayOffset, className }: any) => (
    ...
    >
      {[...images, ...images].map((path, i) => (
        <div key={`${i}`} className="relative aspect-[2/3] w-full overflow-hidden rounded-2xl bg-neutral-800 shadow-xl">
          <Image src={tmdbImage(path as string, 'w500')} alt="" fill className="object-cover" unoptimized />
        </div>
      ))}
    </div>
  )
  ...
  return (
    <div className="relative flex flex-1 h-screen w-full flex-col bg-black overflow-hidden">
      ...
      <div className="absolute inset-0 z-0 opacity-40 md:opacity-30">
        <div className="flex w-full justify-center gap-4 p-4" style={{ transform: 'rotate(-5deg) scale(1.25) translateY(-5%)' }}>
          <Column images={col1} duration={40} delayOffset={10} className="hidden lg:flex" />
          <Column images={col2} duration={45} delayOffset={25} className="hidden md:flex pt-12" />
          <Column images={col3} duration={38} delayOffset={5} className="pt-6" />
          <Column images={col4} duration={42} delayOffset={20} className="" />
          <Column images={col5} duration={35} delayOffset={15} className="pt-16" />
          <Column images={col1} duration={39} delayOffset={8} className="hidden md:flex pt-8" />
          <Column images={col2} duration={48} delayOffset={30} className="hidden lg:flex pt-2" />
        </div>
      </div>
